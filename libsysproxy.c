...
int setProxy(wchar_t * _host, unsigned short length) {
	...
	options.pOptions[1].Value.pszValue = host;
	...
	{*/
		options.pOptions[2].Value.pszValue = _T("localhost;127.*");
	//}
	int ret = apply(&options);
	free(options.pOptions);
	return ret;
